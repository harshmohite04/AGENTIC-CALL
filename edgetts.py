...
import webrtcvad
import logging
import edge_tts  # Edge Text-to-Speech
...
class FreeTTSVoiceAssistant:
    def __init__(self):
        ...
        self.silence_threshold = 15  # Increased silence threshold
        self.audio_buffer = collections.deque(maxlen=MAX_BUFFER_FRAMES)
        self.is_speech_detected = False
        self.silence_frames = 0
        self.speech_frames = 0
        self.consecutive_speech_threshold = 5  # Require more consecutive speech
        ...
        self.validate_tts_voice()

    def validate_tts_voice(self):
        ...
        try:
            ...
            if not any(v['ShortName'] == target_voice for v in voices):
                ...
                if en_voices:
                    self.tts_voice = en_voices[0]['ShortName']
                else:
                    self.tts_voice = voices[0]['ShortName']
                logging.info(f"🔊 Using TTS voice: {self.tts_voice}")
            else:
                self.tts_voice = target_voice
        except Exception as e:
            logging.error(f"❌ Voice validation failed: {e}")
            self.tts_voice = "hi-IN-MadhurNeural"  # or "hi-IN-SwaraNeural"  # Fallback
                ...
                if not self.llm_text_queue.empty() and not self.is_speaking:
                    ...
                    try:
                        ...
                        await asyncio.wait_for(
                            self.synthesize_speech_edgetts(text),
                            timeout=30.0
                        )
    ...
    async def synthesize_speech_edgetts(self, text):
        """TTS using Edge TTS with proper voice selection and encoding"""
        try:
            if self.user_interrupted:
                return
            ...
            async with self.tts_lock:
                ...
                # Use a specific neural voice that supports English
                voice = "hi-IN-MadhurNeural"  # or "hi-IN-SwaraNeural"  # Male voice
                # voice = "en-US-AriaNeural"  # Female voice alternative
