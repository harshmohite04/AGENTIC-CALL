...
class StreamingCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_end(self, response, **kwargs) -> None:
        ...
        if self.full_response.strip() and not self.assistant_ref.user_interrupted:
            logging.info(f"LLM response complete: {self.full_response}")
            self.text_queue.put(self.full_response)
        else:
...
class FreeTTSVoiceAssistant:
    def __init__(self):
        ...
        self.system_prompt = """You are an AI customer care agent for Bansal Classes. Greet the user politely only at the start of the first interaction. Your goal is to engage students/customers in a friendly, professional manner, promoting Bansal Classes effectively to encourage admissions.
        Use abbreviations only when necessary (e.g., API), avoid abbreviating terms like MR, RS.
        Do not entertain requests unrelated to the admission or classes.
        Keep responses concise—preferably within 2 lines unless more detail is essential.
        Focus on highlighting the strengths and benefits of Bansal Classes (quality teaching, experienced faculty, good results, flexible timings, affordable fees, etc.).
        If the student shows interest, politely ask for their details (name, contact info, preferred course) to proceed with admission.
        Avoid controversial or sensitive topics.
        Always base responses strictly on the information provided.
        Be polite, friendly, and encouraging to maximize student retention."""
        ...
        while self.connection_active:
            try:
                if not self.transcript_queue.empty():
                    ...
                    asyncio.create_task(self.handle_llm_prompt(transcript))                    
    ...
    async def handle_llm_prompt(self, transcript):
        if self.user_interrupted:
            return

        logging.info(f"🤖 Sending to LLM: {transcript}")
        context = self.build_conversation_context()
        self.streaming_handler.current_text = ""
        self.streaming_handler.full_response = ""
    ...
    async def stream_tts_audio(self):
        """Convert complete text responses to speech using ElevenLabs TTS"""
        while self.connection_active:
            try:
                if not self.llm_text_queue.empty() and not self.is_speaking:
                    ...
                    try:
                        ...
                        await asyncio.wait_for(
                            self.synthesize_speech_elevenlabs(text),
                            timeout=30.0
                        )
    ...
    async def synthesize_speech_elevenlabs(self, text):
