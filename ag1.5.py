...
# ElevenLabs settings
ELEVENLABS_SAMPLE_RATE = 24000  # ElevenLabs uses 24kHz
ELEVENLABS_VOICE_ID = "pNInz6obpgDQGcFmaJgB"  # Adam voice (you can change this)
...
# ---------- Voice RAG Assistant with ElevenLabs TTS ----------
class VoiceRAGAssistant:
    def __init__(self, qa_chain, deepgram_api_key, elevenlabs_api_key):
        ...
        self.processing_query = False  # Prevent duplicate processing
        self.last_processed_transcript = ""  # Track last processed transcript
        self.processing_lock = asyncio.Lock()  # Async lock for processing
    ...
    async def stream_deepgram_transcription(self):
        ...
        if self.connection_active:
            ...
            if self.connection_active:
                await self.stream_deepgram_transcription()
    ...
    async def text_to_speech_processor(self):
        """Process TTS queue and generate speech using ElevenLabs real-time TTS"""
        while True:
            try:
                if not self.tts_queue.empty() and not self.is_speaking:
                    ...
                    print(f"🔊 Generating speech with ElevenLabs for: {text[:50]}...")
                    
                    # Generate speech using ElevenLabs
                    await self.generate_and_play_elevenlabs_speech(text, language_style)
                    
                await asyncio.sleep(0.1)
            except Exception as e:
                print(f"❌ TTS processing error: {e}")

    async def generate_and_play_elevenlabs_speech(self, text: str, language_style: str):
        """Generate speech using ElevenLabs REST API and play it"""
        try:
            self.is_speaking = True
            print(f"🔊 Starting ElevenLabs speech generation and playback...")
            ...
            data = {
                "text": text,
                "model_id": "eleven_multilingual_v2",  # Changed to multilingual
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        print("🔇 ElevenLabs speech playback completed")
                ...
                print(f"❌ ElevenLabs API connection error: {api_error}")
                
        except Exception as e:
            print(f"❌ ElevenLabs speech generation error: {e}")
        finally:
            audio_data.close()
            # Add delay before re-enabling listening
            await asyncio.sleep(1.0)
            self.is_speaking = False
            print("🎤 Re-enabled listening after ElevenLabs speech")
        
    async def start_voice_interaction(self):
        """Start complete voice interaction with RAG system and ElevenLabs TTS"""
        print("🟢 Starting Voice-Enabled RAG Assistant with ElevenLabs TTS")
        print("🎤 Listening for voice input... Speak naturally!")
        print("🔊 I will speak back to you using ElevenLabs real-time TTS!")
        print("📝 You can also type 'exit' to quit or use Ctrl+C")
        print("🔧 Using ElevenLabs Turbo v2.5 model with real-time WebSocket streaming")
...
# ---------- Main RAG Builder ----------
def build_rag(pdf_path: str, groq_api_key: str, deepgram_api_key: str, elevenlabs_api_key: str, index_save_path: str = "faiss_index"):
    # 1) Try loaders
    ...
    print("[+] Creating embeddings (HuggingFace: sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2). This may download model on first run.")
...
# ---------- Async Main ----------
async def async_main(pdf_path: str, groq_api_key: str, deepgram_api_key: str, elevenlabs_api_key: str, index_save_path: str):
    """Async main function to handle voice RAG with ElevenLabs TTS"""
    try:
        # Build RAG pipeline
        qa_chain = build_rag(pdf_path, groq_api_key, deepgram_api_key, elevenlabs_api_key, index_save_path)
        ...
        print("[+] Voice output enabled with ElevenLabs real-time TTS!")
        print("[+] Using ElevenLabs Turbo v2.5 model for high-quality speech!")
        
        # Create voice assistant with ElevenLabs TTS
        voice_assistant = VoiceRAGAssistant(qa_chain, deepgram_api_key, elevenlabs_api_key)
        
        # Start voice interaction with ElevenLabs TTS
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Voice-enabled RAG with ElevenLabs TTS using Groq + Deepgram + ElevenLabs")
    ...
    try:
        ...
        asyncio.run(async_main(pdf_path, GROQ_API_KEY, DEEPGRAM_API_KEY, ELEVENLABS_API_KEY, args.index))
